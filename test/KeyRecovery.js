...
async function signData(identity, key, address) {
  ...
  const dataToSign = `1900${stripHexPrefix(
    DIDRegistry.address,
  )}${paddedNonce}${stripHexPrefix(identity)}${
    Buffer.from('recover').toString('hex') + stripHexPrefix(address)
  }`;
...
contract('DIDRegistry Recoverable', (accounts) => {
  before(async () => {
    ...
    await instance.addController(accounts[0], addresses[0], {
      from: accounts[0],
    });
    await instance.addController(accounts[0], addresses[1], {
      from: accounts[0],
    });
    await instance.addController(accounts[0], addresses[2], {
      from: accounts[0],
    });
  });
  ...
  it('should not have the minimum keys', async () => {
    ...
    try {
      ...
      await instance.recover(accounts[0], sign.v, sign.r, sign.s, accounts[1], {
        from: accounts[0],
      });
    } catch (e) {
      return assert.equal(
  ...
  it('should fail the recover signature', async () => {
    const instance = await DIDRegistry.deployed();
    try {
      await instance.addController(accounts[0], addresses[3], {
        from: accounts[0],
      });
  ...
  it('should recover key successfully', async () => {
    ...
    const recoveredController = await instance.identityController.call(
      accounts[0],
    );
    ...
    return assert.equal(
      recoveredController.toLowerCase(),
      addresses[2].toLowerCase(),
      'DIDRegistry did add controller',
    );
  });
});
